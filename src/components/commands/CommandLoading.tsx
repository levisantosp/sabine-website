...
export default function CommandLoading() {
  ...
  useEffect(() => {
    const fetchCommands = async() => {
      const commands: Command[] = await (await fetch("/api/commands", {
        headers: {
    ...
    <>
      ...
      >
        {commands.map((cmd, i) => (
          <div
            key={i}
            className="bg-[#2A2A2A] p-5 rounded-2xl max-w-xs md:max-w-3xs mb-6 w-[700] transition duration-500 hover:scale-105"
          >
            <h2
